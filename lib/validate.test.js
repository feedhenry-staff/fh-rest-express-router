...
describe(__filename, function () {
  ...
  beforeEach(function () {
    ...
    app.all('/gadgets', mod({
      validations: {
        list: [Joi.object().keys({
          who: Joi.string().alphanum().min(2).max(15),
          what: Joi.number()
        })]
      },
      joiValidateOptions: {
  ...
  describe('middleware', function () {
    ...
    it('should use the default for who and accept the list request', function (done) {
      request
    ...
    it('should accept the update request, fire twice?', function (done) {
      request
        .put('/gidgets')
